...
usize_t pfc::str_to_float64(float64 &v_, const char *s_)
{
  // skip white spaces
  const char *str=s_;
  ...
  do
  {
    if(is_decimal(*str))
    {
      ...
      {
        dec_fact*=0.1;
        v+=float64(dec_char_to_uint(*str))*dec_fact;
      }
      else
        v=v*10.0+float64(dec_char_to_uint(*str));
      sign=sign?sign:'+';
      is_number=true;
    }
    else
...
usize_t pfc::str_to_int64(int64 &v_, const char *s_)
{
  // skip white spaces
  const char *str=s_;
  while(*str && is_whitespace(*str))
...
int pfc::str_to_scalar64(float64 &vf_, int64 &vi_, const char *s_)
{
  // skip white spaces
  const char *str=s_;
  while(*str && is_whitespace(*str))
  ...
  do
  {
    if(is_decimal(*str))
    {
      ...
      {
        dec_fact*=0.1;
        vd+=10*vd+int64(dec_char_to_uint(*str));
      }
      else
        vi=vi*10+int64(dec_char_to_uint(*str));
      sign=sign?sign:'+';
      is_number=true;
    }
    else
