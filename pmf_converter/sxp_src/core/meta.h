...
template<typename> struct is_type_introspec;     // classes with introspection
template<typename> struct is_type_crep;          // classes with class repository
template<typename> struct is_type_orep;          // classes with object repository
template<typename> struct is_type_var_csig;      // classes with variable class signature (i.e. depends on class instance)
...
template<typename> struct is_ptr_introspec;    // pointers to classes with introspection
template<typename> struct is_ptr_crep;         // pointers to objects with class repository
template<typename> struct is_ptr_orep;         // pointers to objects with object repository
template<typename> struct is_ptr_var_csig;     // pointers to objects with variable class signature
// reference traits
...
template<typename> struct is_ref_introspec;    // references to classes with introspection
template<typename> struct is_ref_crep;         // references to objects with class repository
template<typename> struct is_ref_orep;         // references to objects with object repository
template<typename> struct is_ref_var_csig;     // references to objects with variable class signature
...
// bit math
template<uint32> struct meta_next_pow2;  // next power-of-2 value. if value is power-of-2, returns the value
template<uint32> struct meta_is_pow2;    // check if the value is power-of-2
template<uint32> struct meta_num_bits;   // number of set bits
template<uint32> struct meta_bitpos;     // position of the bit of a power-of-2 value
template<uint32> struct meta_lsb;        // the least significant set bit
template<uint32> struct meta_msb;        // the most significant set bit
template<unsigned size, unsigned pos=0, typename T=uint32, unsigned dw32_idx=0> struct meta_mask;   // generate bit mask of given size and type at given position (i.e. all bits 0, set bits [pos, pos+size-1])
template<unsigned size, unsigned pos=0, typename T=uint32, unsigned dw32_idx=0> struct meta_imask;  // generate inverse bit mask of given size and type at given position (i.e. all bits 1, unset bits [pos, pos+size-1])
template<uint32, int shift> struct meta_bit_shift; // shift bits with signed value: negative values shift to the right and positive to the left
// type size & alignment
...
// core class traits
//============================================================================
PFC_SET_TYPE_TRAIT2(uint128, is_type_pod, is_type_pod_stream, true);
PFC_SET_TYPE_TRAIT(uint128, is_type_class, false);
